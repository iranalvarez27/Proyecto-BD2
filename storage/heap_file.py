...
if __name__ == "__main__":
    ...
    if os.path.exists("heap_test.db"):
        os.remove("heap_test.db")

    schema = Schema(
        table_name="alumnos",
        columns=[
            Column("id", DataType.INT, 4, is_pk=True),
            Column("nombre", DataType.VARCHAR, 200),
            Column("edad", DataType.SMALLINT, 2)
        ]
    )

    heap = HeapFile("heap_test.db")
    ...
    for i in range(100):
        ...
        rid = heap.insert(record, schema)
        rids.append(rid)

    print("Cantidad de paginas:", heap.page_count())

    print("Primer RID:", rids[0])
    print("RID 50:", rids[50])
    print("Ultimo RID:", rids[-1])

    print("\nPrimer registro:")
    print(heap.read(rids[0], schema))

    print("\nUltimo registro:")
    print(heap.read(rids[-1], schema))
    print("\nSCAN COMPLETO:")

count = 0

for record in heap.scan(schema):
    count += 1
...
print("Registros despues de eliminar:", count)