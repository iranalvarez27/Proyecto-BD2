...
class BPlusTree(Index):
    ...
    def bulk_load(self, pairs) -> None:
        ...
        for j, (a, b) in enumerate(runs):
            ...
            level.append((keys[a], self._seg.append(leaf.to_bytes())))
        ...
        while len(level) > 1:
            ...
            for a, b in runs:
                ...
                parents.append((level[a][0], self._seg.append(node.to_bytes())))
            level = parents
            self._height += 1
    ...
    def _read_node(self, page_id: int) -> NodePage:
        return NodePage.from_bytes(self._seg.read(page_id), leaf_codec=self._leaf_codec)
    ...
    def _create(self, key_type: DataType, unique: bool, clustered: bool) -> None:
        ...
        self._seg.append(NodePage(is_leaf=True, leaf_codec=self._leaf_codec).to_bytes())
        self._flush_meta()

    def _load(self) -> None:
        flags, key_type, root, height, payload_size, free_list_head = struct.unpack_from(
            META_FORMAT, self._seg.read(META_PAGE), 0
        )
